...
Use this file only **after** the measurement-validity gate in `alignment-measurement-validity.md`.
...
Do not infer a bent arm, knuckle, frame, or adjuster from camber alone.
...
Before using an individual caster or cross-caster value, verify the per-wheel sweep requirements in `alignment-measurement-validity.md`.
...
Do not tune caster to hide binding.
...
2. identify the actual adjustable mechanism;
3. determine mechanically which other angles that movement changes;
4. make one controlled movement;
5. settle the suspension;
6. remeasure every coupled angle;
7. iterate using the vehicle procedure;
8. finalize the remaining independent settings when the procedure directs.

For the Dakota-specific lower-control-arm cam geometry, see the dedicated section in `alignment-measurement-validity.md` and the current case in `dakota-alignment-case.md`.
